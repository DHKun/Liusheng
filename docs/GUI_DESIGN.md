...
| 正在播放 | 封面与歌词、聚焦歌词、纯封面三种布局；共用常驻播放器 |
| 设置 | 曲库、播放、外观与行为、关于与更新四个分组；保存与取消语义保持清晰 |
...
页面标题通常为 26px，歌曲标题为 14px，辅助信息为 11–12px。默认正文行高 56px，队列行高 64px。常用按钮高度 38px，紧凑按钮高度 32px。键盘焦点使用明确的强调色描边。
...
空间转场使用 `Theme.spatial`（标准 260ms，减少动画为 0ms）；常规控件继续使用 fast/medium/slow。扫描菜单在运行中提供取消入口，停止后恢复重新扫描入口。更多菜单、更新和歌词弹层继续使用主窗口内的 `Popup.Item`。

当前回归范围由 `ValidationHarness.qml`、`tests/qml/` 与共享 `quality.yml` 定义；本文首版测试数字保留其历史基线。后续实现与证据见 [OPTIMIZATION_ITERATION.md](OPTIMIZATION_ITERATION.md)。
