...
## 2026-09-09 · 手动与启动更新检查

使用既有 Qt Network 的异步能力读取 GitHub 最新正式 Release，首帧和设置就绪后再调度。自动路径仅显示窗口内轻提示；手动路径使用 Popup.Item 详情对话框。只做检查与用户确认后的浏览器下载，系统安装及源码安装沿用各自的原流程。

版本、URL 和包选择独立为纯策略，网络状态独立于音频引擎。跳过版本偏好独立保存，避免设置草稿覆盖；响应缓存使用 ETag、大小限制和原子写入。默认 CI 使用隔离本地 HTTP 服务器，Linux 与 macOS arm64 共用测试，应用 QA 模式禁止公开更新请求。见 docs/UPDATES.md。


## 2026-09-10 · 技术债迭代与统一质量门禁

播放顺序由引擎内的 PlaybackOrder 持有；输出切换通过 FIFO 交接屏障取得 PlaybackCheckpoint，保留实际随机顺序及已执行命令的最终位置。UI、MPRIS、macOS 读取统一导航能力。暂停会话持久化可选 playback_order，旧 session v1 使用默认值兼容。

曲库 SQLite 继续单线程写入，文件遍历与标签解析迁至独立有界 I/O worker。目录事件按最小子树核对，单路径通过索引获取状态；取消标志贯穿读入和提交。会话保存期限在处理每条事件时检查，失败的最新检查点保留待重试。读线程若卡在底层文件系统调用，退出会停止接收其结果；该线程只读文件且持有零数据库写权限。

快照以 Arc 共享，单文件变化仅重新读取变化行，保留其他行身份；拼音摘要在工作线程准备，封面只更新关联模型行。组合搜索先分词、再分别归一化。外部歌词异常回退内嵌内容，并设置文本和行数边界。

普通 CI 与标签发布调用同提交的 quality.yml。四种发布产物范围保持，新增真实 DEB/RPM 安装后启动和 macOS ZIP 迁移后启动检查。仓库写权限保留给最终发布 job。物理声卡、GPU、原生系统媒体交互及 Developer ID 公证仍有各自的实机／凭据验收边界。详细见 docs/OPTIMIZATION_ITERATION.md。
